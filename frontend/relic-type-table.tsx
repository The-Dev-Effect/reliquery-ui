...
export const RelicTypeTable = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      const result = await fetch(`${base_path}/api/relic-types`);
    ...
    <Box a="center">
      <Breadcrumb fontWeight="medium" fontSize="sm">
        ...
        <BreadcrumbItem isCurrentPage>
          <BreadcrumbLink href="/dashboard/relic-types">
            Relic types
          </BreadcrumbLink>
        </BreadcrumbItem>
      </Breadcrumb>
            ...
            <LinkBox as={Tr} transform="scale(1)">
              {Object.keys(type).map((key) => (
                <Td>
                  <LinkOverlay
                    isExternal={false}
                    href={`/dashboard/relic-types/${type.storage}/${type.type}`}
                  >
                    {type[key as keyof RelicType]}
                  </LinkOverlay>
                </Td>
              ))}
            </LinkBox>
